    ...
    public class AutoInjectAttributeDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (fieldType.IsSubclassOf(typeof(Component)) == false)
            {
                ...
                EditorHelper.ColorErrorBox(ErrorRect, $"[AutoInject - Error] Component를 상속하는 타입에만 사용할 수 있습니다\n대상 타입 : {fieldType}");
                return;
            }
            ...
            if (property.objectReferenceValue != null)
            {
                EditorHelper.ColorInfoBox(infoRect, Color.green, $"[AutoInject] {Atr.Option}");
            }
            // 실행 결과 - 실패(대상이 없음)
            else
            {
                EditorHelper.ColorWarningBox(infoRect, $"[AutoInject - Failed(Non-Existent Target)] {Atr.Option}");
            }
            EditorGUI.PropertyField(propRect, property, label, true);
        }
    }
